...
def setLight_sun(rotation_euler, light_lcoation, strength, shadow_soft_size = 0.05):
	x = rotation_euler[0] * 1.0 / 180.0 * np.pi 
	y = rotation_euler[1] * 1.0 / 180.0 * np.pi 
	z = rotation_euler[2] * 1.0 / 180.0 * np.pi 
	angle = (x,y,z)
	if light_lcoation is not None:
		bpy.ops.object.light_add(type = 'SUN', rotation = angle, location=light_lcoation)
	else:
