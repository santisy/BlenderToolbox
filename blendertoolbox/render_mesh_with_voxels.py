import os, bpy
import numpy as np

from . blenderInit import blenderInit
from . readMesh import readMesh
from . readNumpyMesh import readNumpyMesh
from . subdivision import subdivision
from . setMat_plastic import setMat_plastic
from . invisibleGround import invisibleGround
from . setCamera import setCamera
from . setLight_sun import setLight_sun
from . setLight_ambient import setLight_ambient
from . shadowThreshold import shadowThreshold
from . renderImage import renderImage
from . render_custom_cube import render_custom_cube

import math
from mathutils import Euler

class colorObj(object):
    def __init__(self, RGBA, \
    H = 0.5, S = 1.0, V = 1.0,\
    B = 0.0, C = 0.0):
        self.H = H # hue
        self.S = S # saturation
        self.V = V # value
        self.RGBA = RGBA
        self.B = B # birghtness
        self.C = C # contrast

def render_mesh_with_voxels(args, voxels, no_plane=False):
  ## initialize blender
  imgRes_x = args["image_resolution"][0]
  imgRes_y = args["image_resolution"][1]
  numSamples = args["number_of_samples"]
  exposure = 1.5 
  use_GPU = True
  blenderInit(imgRes_x, imgRes_y, numSamples, exposure, use_GPU)

  ## read mesh
  location = args["mesh_position"]
  rotation = args["mesh_rotation"]
  scale = args["mesh_scale"]
  if "mesh_path" in args:
    meshPath = args["mesh_path"]
    mesh = readMesh(meshPath, location, rotation, scale)
  elif "mesh_path" not in args and "vertices" in args and "faces" in args:
    V = args["vertices"]
    F = args["faces"]
    mesh = readNumpyMesh(V,F,location,rotation,scale)
  else:
    raise ValueError("one should provide either [mesh_path] or [verticesfaces] in the args")   

  ## set shading (uncomment one of them)
  if args["shading"] == "smooth":
    bpy.ops.object.shade_smooth() 
  elif args["shading"] == "flat":
    bpy.ops.object.shade_flat() 
  else:
    raise ValueError("shading should be either flat or smooth in lazy pipeline")

  ## subdivision
  subdivision(mesh, level = args["subdivision_iteration"])

  ## default render as plastic
  RGB = args["mesh_RGB"]
  RGBA = (RGB[0], RGB[1], RGB[2], 1)
  meshColor = colorObj(RGBA, 0.5, 1.0, 1.0, 0.0, 2.0)
  setMat_plastic(mesh, meshColor)

  ## set invisible plane (shadow catcher)
  if not no_plane:
      ground_location = args.get("ground_location", (0, 0, 0))
      invisibleGround(shadowBrightness=0.9, location=ground_location)

  ## Render voxels
  voxel_transparency = args.get("voxel_transparency", 0.5)
  voxel_color = args.get("voxel_color", (235.0 / 255.0, 94.0 / 255.0, 52.0 / 255.0))
  for voxel in voxels:
      render_custom_cube(voxel, voxel_transparency, voxel_color, location, rotation)

  ## set camera 
  camLocation = args.get("camLocation")
  camRotation = args.get("camRotation")
  focalLength = 45 # (UI: click camera > Object Data > Focal Length)
  bpy.ops.object.camera_add(location = camLocation) # name 'Camera'
  cam = bpy.context.object
  cam.data.lens = focalLength
  x_deg, y_deg, z_deg = camRotation
  cam.rotation_euler = Euler((math.radians(x_deg), math.radians(y_deg), math.radians(z_deg)), 'XYZ')

  ## set light
  lightAngle = args["light_angle"]
  lightLocation = args.get("light_location", None)
  strength = 2
  shadowSoftness = 0.3
  sun = setLight_sun(lightAngle, lightLocation, strength, shadowSoftness)

  ## set ambient light
  setLight_ambient(color=(0.1,0.1,0.1,1)) 

  ## set gray shadow to completely white with a threshold (optional but recommended)
  shadowThreshold(alphaThreshold = 0.05, interpolationMode = 'CARDINAL')

  ## save blender file so that you can adjust parameters in the UI
  bpy.ops.wm.save_mainfile(filepath=os.getcwd() + '/test.blend')

  ## save rendering
  renderImage(args["output_path"], cam)
